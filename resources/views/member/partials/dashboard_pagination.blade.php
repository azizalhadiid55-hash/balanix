...
<ul class="pagination" id="pagination-dashboard">
    {{-- Previous --}}
    <li class="page-item {{ $transaksi->onFirstPage() ? 'disabled' : '' }}">
        ...
        @else
            <a class="page-link"
               ...
               data-ajax="{{ route('dashboard.list', array_merge(request()->only('q','filter'), ['page' => $transaksi->currentPage() - 1])) }}">
               Previous
            </a>
        @endif
    </li>
        ...
        <li class="page-item {{ $page == $transaksi->currentPage() ? 'active' : '' }}">
            ...
            @else
                <a class="page-link"
                   ...
                   data-ajax="{{ route('dashboard.list', array_merge(request()->only('q','filter'), ['page' => $page])) }}">
                   {{ $page }}
                </a>
            @endif
        </li>
